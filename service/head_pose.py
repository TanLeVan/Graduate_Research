...
class HeadPoseEstimator:
    ...
    def __init__(self, filepath, W, H) -> None:
        ...
        self.cam_matrix = np.array([[W, 0, W/2.0],
                                    [0, W, H/2.0],
                                    [0, 0, 1]])
        """need to perform camera calibration and change the values of this matrix"""
    ...
    def get_head_pose(self, shape):
        ...
        """this function return the rotation and translation vector of the object in the carmera coordinate Frame. """
        """Change the cameraMatrix parameter. cameraMatrix parameter is the internal matrix of the camera """
        ret, rotation_vec, translation_vec = cv2.solvePnP(
            self.object_pts,
            image_pts,
            cameraMatrix=self.cam_matrix,
            distCoeffs=None, # after calibration, maybe need to change the distortion coefficients
            rvec=self.r_vec,
            tvec=self.t_vec,
        ...
        top_height = -90
        right_width = 75
        bottom_height = 90 #unit is mm. speculation only
        ...
        reprojectdst, _ = cv2.projectPoints(reprojectsrc,
                                            ...
                                            self.cam_matrix,
                                            distCoeffs=None)
        """Returns the points in pixel coordinate from 3d camera coordintate"""
        ...
        #need to understand the Euler angle
        return reprojectdst, euler_angle, rotation_mat,rotation_vec, translation_vec
...
def main(filename):
    ...
    #cap.get(3) return the width of the frame. cap.get(4) return the height of the frame
    color = (125, 255, 125)
    ...
    while True:
        ...
        for pred in fa.get_landmarks(frame, bboxes, True):
            ...
            reprojectdst, euler_angle,_,_, _= hp.get_head_pose(pred)
